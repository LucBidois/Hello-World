...
class Square():
    ...
    def GLDraw_Square_Front(self):
        """PLEASE NOTE THIS DRAWS FRONT FACE ONLY"""
        j = int(self.row)
        i = int(self.col)
    ...
    def GLDraw_Square_Right(self):
        ...
        glColor3fv(self.colour)
        glBegin(GL_QUADS)
        glVertex3f(n, n-j-1, i)
        glVertex3f(n, n-j, i)
        glVertex3f(n, n-j, i+1)
        glVertex3f(n, n-j-1, i+1)
        glEnd()
    ...
    def GLDraw_Square_Top(self):
        ...
        glColor3fv(self.colour)
        glBegin(GL_QUADS)
        glVertex3f(i, n, n-j-1)
        glVertex3f(i, n, n-j)
        glVertex3f(i+1, n, n-j)
        glVertex3f(i+1, n, n-j-1)
        glEnd()
...
class Cube(object):
    ...
    def Face_Spin_Anticlockwise(self, face):
        ...
        for j in range(0, n):
            colour_Index = (n-1) - j
            for i in range(0, n):
                self.faces[face].squares[i + (n-1)*j].colour = old_Colours[colour_Index]
...
def main():
    ...
    glTranslate(-5, -5, -15) #moves camera back 5 units z direction
    ...
    while True:
        ...
        Rubiks.GL_Draw_Cube()
        #Rubiks.faces[0].GL_Draw_Face_Front()
        #Rubiks.GL_Square_Sepearation_Lines()
        """GL Test drawings"""
